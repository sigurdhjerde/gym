...
class HovorkaCambridgeBase(gym.Env):
    ...
    def __init__(self):
        ...
        P, init_basal_optimal, optimal_bolus = hovorka_cambridge_pars(0)
        self.P = P
    ...
    def change_patient(self, pat_num):
        '''
        Changing the patient parameters
        '''
    ...
    def step(self, action):
        ...
        if not done:
            if self.reward_flag != 'asymmetric_insulin':
                reward = rewardFunction.calculate_reward(np.array(bg), self.reward_flag, 108)
            else:
                reward = rewardFunction.calculate_reward(np.array(bg), 'asymmetric_insulin', 108, action)
