...
class HovorkaIntervalHovorka(gym.Env):
    # TODO: fix metadata??
    metadata = {
    ...
    def __init__(self):
        ...
        self.bg_history = np.repeat(initial_bg, self.simulation_time)
        self.insulin_history = np.repeat(initial_insulin, self.simulation_time)
    ...
    def _step(self, action):
        ...
        # Noting the last value of the previous state -- used for the reward
        previous_bg_last = self.bg_history[-1]

        # Recording bg history for plotting
        self.bg_history = np.concatenate([self.bg_history, bg])
        self.insulin_history = np.concatenate([self.insulin_history, insulin])
        ...
        if not done:
            reward = calculate_reward(np.array(bg), reward_flag=self.reward_flag, bg_ref=108, blood_glucose_level_start=previous_bg_last)
